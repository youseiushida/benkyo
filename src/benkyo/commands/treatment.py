"""treatment subcommand (per (project, concept) procedural/conceptual annotation)."""
...
TREATMENTS = ["procedural", "conceptual"]
SET_BY = ["system", "learner", "material"]


@click.group(name="treatment")
def treatment_group():
    """Manage per-project concept treatment (procedural / conceptual)."""
...
@click.option("--reference", default=None, help="Reference content when procedural")
...
def get(ctx, project_id, concept_id):
    """Get the treatment (defaults to conceptual if unset)."""
...
def unset(ctx, project_id, concept_id):
    """Remove explicit setting (reverts to default conceptual)."""
