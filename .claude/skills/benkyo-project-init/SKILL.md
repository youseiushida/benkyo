...
This skill handles the project-creation phase: extracting a concrete goal from the learner, ingesting materials, drafting the initial concept graph, and deciding the initial procedural/conceptual cut.
...
- procedural → 「公式扱い」「道具として使う」「暗記でいい」
- conceptual → 「ちゃんと理解する」「掘り下げる」「腹落ちさせる」
...
4. **Set the initial cut** (which concepts are procedural, which are conceptual).
...
2. **Extract structure**:
   ...
   - Sections that teach via tables/formulas only → procedural treatment candidates
   - Sections with derivation/proof → conceptual treatment candidates
...
For each concept in the project, decide: procedural or conceptual?
...
The user confirms; the tutor sets procedural treatment for the agreed items.

### lazy-prune (when no materials)

Set everything to default conceptual; let release happen during sessions as the learner expresses "ここは公式でいい":
...
### Either way, default for unset is conceptual

The project_concepts table is sparse: only explicit treatments are stored. Unset = conceptual.

To set procedural treatment with reference content:

```
benkyo treatment set --project prj1 --concept c5 \
  --treatment procedural --reference-file laplace-table.md
```
...
Tutor: ざっくりこんな感じで進めよう:
  ...
  - 「公式でいい」扱い: [procedural items]
  - 「ちゃんと理解する」扱い: [conceptual items]
...
# edges drawn
# treatment set for procedural items with reference
```
...
If learner is fluent: those prereqs can stay procedural (or even be marked as procedural with explicit reference).
If learner is shaky: prereqs may need to be promoted to conceptual, which means breakdown will visit them. Adjust the cut.
